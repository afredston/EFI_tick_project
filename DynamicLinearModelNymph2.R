...
out.burn <- window(jags.out, start = burnin)
...
forecastN <- function(IC, beta, rho, Q=tau_add, n=Nmc, NT){
  N <- matrix(NA,n,NT)  ## storage: NT = number of time steps forecasted, n = Nmc = number of ensembles
  Nprev <- IC           ## initialize
  for(t in 2:NT){
...
N.deterministic <- forecastN(IC=mean(IC),
                             beta=out.mat[,"beta"],
                             rho=(data.val$dailyMaxAirTemp-mean(data.val$dailyMaxAirTemp, na.rm=TRUE)),
                             Q=1,
                             NT=NT,
                             n=5)
N.deterministic
