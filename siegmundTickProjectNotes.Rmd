...
fitRandomWalk <- function(data = adultDataWithMet, 
                          siteName = "SERC", jagsModel = RandomWalk,  forecast=FALSE){
...
```{r}
jags.out <- fitRandomWalk(data = adultDataWithMet, 
                          siteName = "ORNL", jagsModel = RandomWalk, forecast=TRUE)
...
ggplot() +
  geom_ribbon(data=sum.data,aes(x=time,ymin=X2.5.,ymax=X97.5.),
              color="cornflowerblue",fill="cornflowerblue",alpha=0.5) +
  geom_line(data=sum.data,aes(x=time,y=X50.),color="black") +
  geom_point(data=dataForPlotting,aes(x=day,y=totalAdult,color=trainValidate)) +
